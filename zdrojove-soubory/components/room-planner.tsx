...
import { Box, Check, ClipboardCheck, Expand, Grid2X2, House, Layers3, Minus, MousePointer2, PlugZap, Plus, Redo2, RotateCcw, RotateCw, Send, Settings2, Undo2, X } from 'lucide-react';
...
import { RoomItemPanel } from './room-item-panel';
...
import type { useRoomDesign } from '@/hooks/use-room-design';
import { useIsMobile } from '@/hooks/use-mobile';
import { findFreePosition, issuesFor, newFurniture, normalizeItem, placeItem, resizeRoom, upgradeDesign, type Furniture, type FurnitureType, type Issue, type RoomDesign, type Selection } from '@/lib/room';
...
const sections=[{id:'room',name:'Prostor',icon:House},{id:'furniture',name:'Nábytek',icon:Layers3},{id:'technical',name:'Technické prvky',icon:PlugZap},{id:'check',name:'Kontrola',icon:ClipboardCheck},{id:'inquiry',name:'Poptávka',icon:Send}] as const;
type Section=typeof sections[number]['id'];

export type RoomController=Omit<ReturnType<typeof useRoomDesign>,'history'|'dispatch'> & {history:{past:unknown[];future:unknown[]};dispatch:(action:{type:'undo'|'redo'})=>void};
export default function RoomPlanner({controller,modeTabs,designMenu,onEdit,onStartSingle,initialSelectedId}:{controller:RoomController;modeTabs:ReactNode;designMenu:ReactNode;onEdit:(id:string)=>void;onStartSingle:(type:FurnitureType)=>void;initialSelectedId?:string}) {
  const {history,design,dispatch,ready,storageError,saved,recoverSaving,change}=controller;
  const [section,setSection]=useState<Section>(initialSelectedId?'furniture':'room'),[selection,setSelection]=useState<Selection>(initialSelectedId?{kind:'furniture',id:initialSelectedId}:null);
  ...
  const panelTrigger=useRef<HTMLElement|null>(null),isMobile=useIsMobile();
  ...
  function navigate(next:Section) {panelTrigger.current=document.activeElement as HTMLElement;setSection(next);setPlacing(null);setMobileOpen(isMobile);}
  function select(next:Selection,openPanel=true) {
    if(openPanel)panelTrigger.current=document.activeElement as HTMLElement;
    setSelection(next);
    if(next)setSection(next.kind==='technical'?'technical':next.kind==='opening'?'room':'furniture');
    setMobileOpen(isMobile&&!!next&&openPanel);
  }
  ...
  const panel=section==='room'?<div className="tw-panel-content"><RoomControls key={selectedOpening?.id??'room'} room={design.room} selectedOpeningId={selectedOpening?.id} onSelectOpening={id=>select({kind:'opening',id})} onChange={patch=>change(d=>resizeRoom(d,patch))}/><div className="tw-panel-actions"><Button variant="outline" onClick={()=>setChooser(true)}>Vybrat sestavu</Button><Button variant="ghost" onClick={()=>setNewDialog(true)}>Vyprázdnit pokoj</Button></div></div>
    :section==='furniture'?selected?<><RoomItemPanel key={selected.id} item={selected} room={design.room} issues={issues} onBack={()=>setSelection(null)} onEdit={()=>onEdit(selected.id)} onChange={patch=>updateItem(selected.id,patch)} onDuplicate={duplicate} onDelete={()=>{change(d=>({...d,items:d.items.filter(i=>i.id!==selected.id)}));setSelection(null);}}/><FurnitureConnections item={selected} design={design} onBundle={kind=>addBundle(kind,selected.id)} onSelect={id=>select({kind:'technical',id})}/></>:<FurnitureCatalog design={design} onAdd={add} onSelect={id=>select({kind:'furniture',id})} onTemplates={()=>setChooser(true)}/>
  ...
  return <div className="rp-app toro-app tw-app">
    ...
    <main className="tw-main"><nav className="toro-workflow" aria-label="Postup návrhu">{sections.map(item=><button key={item.id} aria-pressed={section===item.id} onClick={()=>navigate(item.id)}><item.icon size={19}/><span>{item.name}{item.id==='check'&&issues.length>0&&<span className="tw-count">{issues.length}</span>}</span></button>)}</nav>
      ...
      <Button className="tw-mobile-tools" variant="outline" onClick={e=>{panelTrigger.current=e.currentTarget;setMobileOpen(true);}}><Settings2/> {sections.find(s=>s.id===section)?.name}</Button></section></div>
      <footer className="tw-footer"><span>Prototyp TORO · nic se neodesílá</span></footer></main>
    <Sheet open={isMobile&&mobileOpen} onOpenChange={setMobileOpen}><SheetContent onCloseAutoFocus={e=>{e.preventDefault();if(panelTrigger.current?.isConnected)panelTrigger.current.focus();}} side="bottom" className="tw-mobile-sheet toro-app" showCloseButton={false}><SheetHeader><SheetTitle>{sections.find(s=>s.id===section)?.name}</SheetTitle><SheetDescription className="sr-only">Nástroje pro úpravu návrhu pokoje</SheetDescription><SheetClose asChild><Button className="tw-sheet-close" variant="ghost" size="icon" aria-label="Zavřít panel"><X/></Button></SheetClose></SheetHeader><div className="tw-sheet-body">{isMobile&&panel}</div></SheetContent></Sheet>
    <ProjectChooser open={chooser} onOpenChange={setChooser} onTemplate={chooseTemplate} onEmpty={()=>startDesign({...createTemplate('hall'),title:'Můj pokoj',items:[]})} onSingle={type=>{setChooser(false);onStartSingle(type);}} onService={()=>{setChooser(false);openInquiry(true);}}/>
